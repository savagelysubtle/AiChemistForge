...
process.stderr.write(
  `SERVER.TS BRAVE_API_KEY AT VERY START: ${
...
log(
  'warn',
  `API Key check after dotenv: ${
    process.env.BRAVE_API_KEY ? 'LOADED' : 'MISSING'
  }`,
);
// ----------------------
log(
  'info',
  `BRAVE_API_KEY from process.env after dotenv: ${
    process.env.BRAVE_API_KEY ? 'Loaded' : 'NOT LOADED'
  }`,
);
...
import {
  CallToolResult,
  TextContent,
} from '@modelcontextprotocol/sdk/types.js'; // Import types for wrapper
import { z } from 'zod';

// Import the tool execution functions and schemas
import {
  executeCodeSearch,
  executeMultiLinkSearch,
  executeWebSearch,
} from '../tools/braveSearchTools.js';

// Import Perplexity Deep Research tool
import { executeDeepResearch } from '../tools/perplexityDeepResearch.js';
...
async function wrapToolExecution(
  ...
  try {
    ...
    log(
      'error',
      `Unexpected error thrown during ${toolName} execution:`,
      executionError,
    );
    return {
      content: [
        {
          type: 'text',
          text: `Unexpected server error during ${toolName}: ${
...
async function main() {
  ...
  try {
    const server = new McpServer({
      ...
      description:
        'MCP Server providing Brave Search and Windows CLI execution tools.',
      capabilities: {
    ...
    server.registerTool(
      ...
      async (args) =>
        wrapToolExecution('brave_web_search', executeWebSearch, args),
    );
    ...
    server.registerTool(
      'brave_code_search',
      {
        ...
        inputSchema: {
          query: z
            .string()
            .describe(
              "Code search query (e.g. 'github repository for brave search')",
            ),
          count: z
            .number()
            .default(10)
      ...
      async (args) =>
        wrapToolExecution('brave_code_search', executeCodeSearch, args),
    );

    log('info', 'Registering tool: multi_link_search');
    server.registerTool(
      'multi_link_search',
      {
        description:
          'Performs sequential web searches for multiple queries or links using the Brave Search API. ' +
          'Ideal for batch information gathering across diverse topics. ' +
          "Executes searches sequentially with delays to respect the 1 req/sec rate limit of Brave's free tier. " +
          'Aggregates results into a single output with clear separation per query. ' +
          'Supports up to 10 queries, with automatic retry logic and error isolation per search. ' +
          'Maximum 20 results per query. Note: Processing time is approximately 1.1 seconds per query.',
        inputSchema: {
          queries: z.array(
            z
              .string()
              ...
              .max(400)
              .describe(
                'Array of search queries or links (1-10 items, each max 400 chars). Executed sequentially to respect rate limits.',
              ),
          ),
          count: z
            .number()
        ...
        annotations: {
          title: 'Multi-Link Search',
        },
      },
      async (args) =>
        wrapToolExecution('multi_link_search', executeMultiLinkSearch, args),
    );

    // Register Perplexity Deep Research tool
    log('info', 'Registering tool: perplexity_deep_research');
    server.registerTool(
      'perplexity_deep_research',
      {
        description:
          'Performs comprehensive deep research using Perplexity AI\'s sonar-deep-research model. ' +
          'Conducts exhaustive searches across hundreds of sources, synthesizing expert-level insights ' +
          'and generating detailed reports with comprehensive analysis and citations. ' +
          'Ideal for in-depth research, market analysis, technical investigations, and academic synthesis. ' +
          'Note: Processing time is 30-90 seconds per query. Maximum query length: 2000 characters.',
        inputSchema: {
          query: z
            .string()
            .min(10)
          ...
          search_recency_filter: z
            ...
            .optional()
            .describe('Filter sources by recency (month/week/day/hour)'),
          return_images: z
            .boolean()
            .default(false)
            .optional()
      ...
      async (args) =>
        wrapToolExecution('perplexity_deep_research', executeDeepResearch, args),
    );
    ...
    server.registerTool(
      ...
      {
        description:
          'Execute a Windows command line command safely with validation and security controls.',
        inputSchema: {
          shell: z
          ...
          dryRun: z
            .boolean()
            .optional()
            .describe('Preview command without executing'),
          force: z
            .boolean()
            .optional()
            .describe('Force execution of destructive commands'),
        },
        annotations: {
      ...
      async (args) =>
        wrapToolExecution('execute_command', executeCommandTool, args),
    );
    ...
    server.registerTool(
      ...
      async (args) =>
        wrapToolExecution('get_command_history', getCommandHistoryTool, args),
    );
    ...
    server.registerTool(
      ...
      {
        description:
          'Get comprehensive information about the current working directory, workspace detection, and directory stack.',
        inputSchema: {},
        annotations: {
    ...
    server.registerTool(
      ...
      {
        description:
          'Intelligently change the working directory with workspace detection and path validation.',
        inputSchema: {
          path: z.string().describe('Target directory path'),
          relative: z
            .boolean()
            .optional()
            .describe('Whether the path is relative to current directory'),
        },
        annotations: {
      ...
      async (args) =>
        wrapToolExecution('change_directory', changeDirectoryTool, args),
    );
    ...
    server.registerTool(
      ...
      {
        description:
          'Find and analyze workspace information starting from a given directory, detecting project types and configuration files.',
        inputSchema: {
          startPath: z
            .string()
            .optional()
            .describe(
              'Starting path to search for workspace (defaults to current directory)',
            ),
        },
        annotations: {
      ...
      async (args) =>
        wrapToolExecution('find_workspace', findWorkspaceTool, args),
    );

    log('info', `All tools registered.`);

    // Initialize search service (pre-loads model if caching enabled)
    if (process.env.ENABLE_SEMANTIC_CACHE === 'true') {
      try {
        ...
        log('warn', 'Failed to load embedding model, caching disabled', error);
      }
    }
