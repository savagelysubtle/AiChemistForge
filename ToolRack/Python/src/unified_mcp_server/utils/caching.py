...
class CacheManager:
    ...
    async def warmup(
        self,
        warmup_functions: Optional[List[Callable[[], Any]]] = None,
        cache_name: str = "default",
    ) -> int:
        """Warm up cache by pre-executing functions.

        Args:
            warmup_functions: List of async functions to execute for cache warming
            cache_name: Name of the cache to warm up

        Returns:
            Number of cache entries created
        """
        if warmup_functions is None:
            warmup_functions = []
        ...
        for func in warmup_functions:
            try:
                ...
                import logging
                logger = logging.getLogger("mcp.utils.caching")
                logger.warning(f"Cache warmup function {func.__name__} failed: {e}")
