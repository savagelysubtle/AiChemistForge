...
def register_codebase_ingest_tool(mcp: FastMCP) -> None:
    ...
    async def codebase_ingest(
        ...
        Processes codebase files and structures them for efficient LLM consumption with token management,
        component extraction, and complexity analysis.

        IMPORTANT: Use FULL PATHS for specific workspaces. "." uses MCP server's directory.

        Args:
            path: Root directory path to ingest (use full paths for specific workspaces) (default: ".")
            max_file_size: Maximum file size in bytes to include (default: 1048576)
            include_binary: Whether to attempt reading binary files (default: False)
            output_format: Output format: 'structured' or 'markdown' (default: 'structured')
            include_patterns: List of glob patterns to include (default: None)
            ...
            chunk_strategy: Strategy for chunking files: 'intelligent', 'simple', or 'none' (default: 'intelligent')
...
"""Codebase ingestion tool for the unified MCP server."""

import logging
from pathlib import Path
from typing import Any, Dict, List, Optional

logger = logging.getLogger("mcp.tools.filesystem")
...
async def ingest_codebase(
    ...
    """Ingest and analyze multiple files from a codebase.
    ...
    Returns:
        Dictionary containing ingested file contents and analysis
    """
    try:
        ...
        for file_path_str in file_paths:
            try:
                ...
                if not file_path.exists():
                    errors.append(f"File does not exist: {file_path_str}")
                    continue
                ...
                if file_size > max_file_size:
                    errors.append(
                        f"File too large: {file_path_str} ({file_size} bytes)"
                    )
                    continue
                ...
                errors.append(f"Error processing {file_path_str}: {str(e)}")
        ...
        logger.error(f"Error in codebase ingestion: {e}")
