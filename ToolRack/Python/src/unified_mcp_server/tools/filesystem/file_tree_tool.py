...
def register_file_tree_tool(mcp: FastMCP) -> None:
    ...
    async def file_tree(
        ...
        Use for project analysis, code review, understanding structure, planning refactoring,
        or debugging. Provides token counting, complexity analysis, and pattern filtering.

        IMPORTANT: Use FULL PATHS for specific workspaces. "." uses MCP server's directory,
        not the agent's current directory.

        Args:
            path: Directory path to analyze (use full paths for specific workspaces)
            max_depth: Maximum depth to traverse (default: 5)
            show_hidden: Whether to show hidden files (default: False)
            show_sizes: Whether to show file sizes (default: True)
            format: Output format 'tree', 'json', or 'flat' (default: 'tree')
            include_patterns: List of glob patterns to include
            exclude_patterns: List of glob patterns to exclude
            show_tokens: Whether to show token counts (default: True)
            show_components: Whether to show component counts (default: True)
            llm_optimized: Whether to optimize for LLM usage (default: True)
            max_context_tokens: Maximum number of tokens in context (default: 100000)
            complexity_filter: Filter for complexity levels: 'all', 'simple', 'moderate', 'complex' (default: 'all')
        """
        logger.debug(f"Starting comprehensive file tree generation for path: {path}")
        ...
        try:
            ...
            if format not in ["tree", "json", "flat"]:
                raise ValueError("format must be 'tree', 'json', or 'flat'")

            if complexity_filter not in ["all", "simple", "moderate", "complex"]:
                raise ValueError(
                    "complexity_filter must be 'all', 'simple', 'moderate', or 'complex'"
                )
