...
/**
 * Ensures a filename has a .md extension
 */
...
/**
 * Validates that a path is within the specified vault
 */
export function validateVaultPath(vaultPath: string, targetPath: string): void {
  ...
  if (!normalizedTarget.startsWith(normalizedVault)) {
    throw new McpError(
      ErrorCode.InvalidRequest,
      `Path is outside vault boundaries: ${targetPath}`
    );
  }
}

/**
 * Handles filesystem errors and converts them to McpErrors
 */
export function handleFsError(error: unknown, operation: string): McpError {
  if (error instanceof McpError) {
    return error;
  }
 ...
 */
export function createNoteNotFoundError(notePath: string): McpError {
  return new McpError(
